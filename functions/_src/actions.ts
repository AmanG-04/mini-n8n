...
export async function createRun(workflowId: string, userId: string, triggerType: "manual" | "webhook", input: object = {}): Promise<StartResult> {
  try {
    const data = await hasura<{ start_workflow_run: StartResult[] }>(
      `mutation Start($workflow: uuid!, $user: uuid!, $trigger: String!, $input: jsonb!) { start_workflow_run(args: { p_workflow_id: $workflow, p_user_id: $user, p_trigger_type: $trigger, p_input: $input }) { run_id status } }`,
...
export async function approveRunStep(stepRunId: string, userId: string): Promise<StartResult> {
  try {
    const data = await hasura<{ approve_workflow_step: StartResult[] }>(
      `mutation Approve($step: uuid!, $user: uuid!) { approve_workflow_step(args: { p_step_run_id: $step, p_user_id: $user }) { run_id status } }`, { step: stepRunId, user: userId }
    );
...
export async function createWebhookRun(triggerId: string, secret: string, input: object = {}): Promise<StartResult> {
  try {
    const data = await hasura<{ start_webhook_workflow_run: StartResult[] }>(
      `mutation Webhook($trigger: uuid!, $secret: String!, $input: jsonb!) { start_webhook_workflow_run(args: { p_trigger_id: $trigger, p_secret: $secret, p_input: $input }) { run_id status } }`, { trigger: triggerId, secret, input }
    );
  ...
  setTimeout(() => {
    void new WorkflowExecutor().execute(runId).then(async (result) => {
      if (result.status === "completed") {
        await hasura(`mutation Usage($run: uuid!) { record_workflow_usage(args: { p_run_id: $run }) { usage_id } }`, { run: runId });
      }
