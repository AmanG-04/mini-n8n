...
BEGIN
  ...
  IF v_org_id IS NULL OR v_role = 'viewer' THEN
  ...
  SELECT
    o.quota_limit,
    COALESCE(SUM(u.billable_calls) FILTER (WHERE u.created_at >= o.quota_period_start), 0)::integer
  INTO v_limit, v_used
  ...
  SELECT v_run.id, id, position, type
...
BEGIN
  ...
  IF v_workflow_id IS NULL THEN
    RAISE EXCEPTION 'WEBHOOK_FORBIDDEN';
  END IF;
  ...
  VALUES (v_workflow_id, v_org_id, 'webhook', p_input)
  RETURNING * INTO v_run;
...
BEGIN
  ...
  WHERE sr.id = p_step_run_id
    AND sr.type = 'approval_gate'
    AND sr.status = 'paused'
    AND wr.status = 'paused'
    AND m.user_id = p_user_id
  FOR UPDATE OF sr, wr;

  IF v_run_id IS NULL OR v_role = 'viewer' THEN
    RAISE EXCEPTION 'APPROVAL_FORBIDDEN';
  END IF;

  UPDATE public.step_runs
  SET approved_by = p_user_id, approved_at = now(), status = 'pending'
