...
export default function Home() {
  ...
  async function loadStepRuns(id: string) { try { const data = await graphQL<{ step_runs: Step[] }>(STEP_RUNS_QUERY, { id }, token); setStepRuns(data.step_runs); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to load execution status."); } }
  useEffect(() => { if (!runId || !token) return; void loadStepRuns(runId); const ws = new WebSocket((process.env.NEXT_PUBLIC_NHOST_GRAPHQL_WS_URL ?? "ws://localhost:1337/v1/graphql").replace("http", "ws"), "graphql-transport-ws"); ws.onopen = () => { ws.send(JSON.stringify({ type: "connection_init", payload: { headers: { authorization: `Bearer ${token}` } } })); }; ws.onmessage = (event) => { const data = JSON.parse(event.data); if (data.type === "connection_ack") { ws.send(JSON.stringify({ id: "steps", type: "subscribe", payload: { query: STEP_RUNS, variables: { id: runId } } })); } else if (data.type === "error") { setMessage("Live subscription error; showing the latest execution state."); } else if (data.payload?.data) { setStepRuns(data.payload.data.step_runs); if (["completed", "failed"].includes(data.payload.data.workflow_runs_by_pk?.status)) void loadWorkflows(); } }; ws.onerror = () => setMessage("Live subscription unavailable; showing the latest execution state."); return () => ws.close(); }, [runId, token]);
