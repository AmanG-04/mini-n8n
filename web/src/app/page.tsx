...
export default function Home() {
  ...
  async function loadStepRuns(id: string) { try { const data = await graphQL<{ step_runs: Step[]; workflow_runs_by_pk: { status: string } | null }>(STEP_RUNS_QUERY, { id }, token); setStepRuns(data.step_runs); if (["completed", "failed"].includes(data.workflow_runs_by_pk?.status ?? "")) void loadWorkflows(); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to load execution status."); } }
