...
export default function Home() {
  ...
  async function editStep(step: Step) { const raw = window.prompt("Step config JSON", JSON.stringify(step.config)); if (raw === null) return; try { const config = JSON.parse(raw); await graphQL(mutation("EditStep", `($id:uuid!,$config:jsonb!){update_workflow_steps_by_pk(pk_columns:{id:$id},_set:{config:$config}){id}}`), { id: step.id, config }, token); await loadWorkflows(); } catch { setMessage("Config must be valid JSON."); } }
