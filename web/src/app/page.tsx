...
import { graphQL, ORGANIZATIONS, WORKFLOWS, STEP_RUNS, STEP_RUNS_QUERY } from "../lib/graphql";
...
export default function Home() {
  ...
  const [selected, setSelected] = useState<Workflow | null>(null); const [runId, setRunId] = useState(""); const [stepRuns, setStepRuns] = useState<StepRun[]>([]); const [message, setMessage] = useState(""); const [usage, setUsage] = useState({ quota_limit: 0, calls_used: 0, calls_remaining: 0 });
  ...
  async function loadStepRuns(id: string) { try { const data = await graphQL<{ step_runs: StepRun[]; workflow_runs_by_pk: { status: string } | null }>(STEP_RUNS_QUERY, { id }, token); setStepRuns(data.step_runs); if (["completed", "failed"].includes(data.workflow_runs_by_pk?.status ?? "")) void loadWorkflows(); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to load execution status."); } }
  useEffect(() => { if (!runId || !token) return; void loadStepRuns(runId); const refresh = window.setInterval(() => void loadStepRuns(runId), 1000); const ws = new WebSocket((process.env.NEXT_PUBLIC_NHOST_GRAPHQL_WS_URL ?? "ws://localhost:1337/v1/graphql").replace("http", "ws"), "graphql-transport-ws"); ws.onopen = () => { ws.send(JSON.stringify({ type: "connection_init", payload: { headers: { authorization: `Bearer ${token}` } } })); }; ws.onmessage = (event) => { const data = JSON.parse(event.data); if (data.type === "connection_ack") { ws.send(JSON.stringify({ id: "steps", type: "subscribe", payload: { query: STEP_RUNS, variables: { id: runId } } })); } else if (data.type === "error") { setMessage("Live subscription error; automatic status refresh is active."); } else if (data.payload?.data) { setStepRuns(data.payload.data.step_runs); if (["completed", "failed"].includes(data.payload.data.workflow_runs_by_pk?.status)) void loadWorkflows(); } }; ws.onerror = () => setMessage("Live subscription unavailable; automatic status refresh is active."); return () => { window.clearInterval(refresh); ws.close(); }; }, [runId, token]);
