...
export const STEP_RUNS_QUERY = `query StepRuns($id: uuid!) { step_runs(where:{workflow_run_id:{_eq:$id}},order_by:{position:asc}) { ${STEP_RUN_FIELDS} } workflow_runs_by_pk(id:$id) { id status error } }`;
