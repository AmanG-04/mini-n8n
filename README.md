...
1. Install and start the Nhost local stack, then apply the committed `nhost/migrations` and `nhost/metadata` using your normal Nhost/Hasura migration workflow. The root `config.yaml` is required by Nhost's hosted migration runner. Do not use a database URL in the browser.
