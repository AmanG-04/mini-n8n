...
2. Nhost automatically provides `NHOST_GRAPHQL_URL` and `NHOST_ADMIN_SECRET` to Functions. In the current Nhost Cloud dashboard, add `GROQ_API_KEY` under **Project Dashboard -> Settings -> Secrets** (there is no per-function secrets page). Set `WORKFLOW_ACTIONS_URL` and `NOTIFICATION_DISPATCH_URL` under **Settings -> Environment Variables** to the deployed/local function URLs. No direct `DATABASE_URL` is needed, so this works on the Nhost Free plan. `llm_call` always makes a real Groq request using `llama-3.3-70b-versatile`; a missing key fails the step explicitly.
