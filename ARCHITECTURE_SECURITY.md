...
`notification_outbox` is delivered by the committed Hasura Event Trigger configuration. The event target reads its outbound endpoint only from the function environment. The client subscribes to `step_runs` by `workflow_run_id`; its Hasura select permission still applies to that subscription. Conditional branches can declare `if_positions` and `else_positions` in their JSON config; the executor marks positions in the non-selected branch as `skipped`, preserving one `step_run` per authored step. Webhook secrets are generated only by the owner-authorized Action, shown once, and stored only as SHA-256 hashes.
