...
`triggerWorkflowRun` creates the run and all ordered `step_runs` in one transaction, then queues the server-side executor. The executor marks each step running/completed/failed, passes the prior output to the next step, retries external Groq/HTTP failures once with a timeout, and writes error details on failure. An `approval_gate` changes both the step and run to `paused`; no later step runs. `approveStep` marks the gate with approver identity/time and queues the same executor, which completes the approved gate and continues.
